...
namespace InvoicingService.Controllers
{
    ...
    public class InvoiceHealthController : ControllerBase
    {
        ...
        /// <summary/>
        public InvoiceHealthController(IInvoiceHealthService healthService, IMapper mapper, IConfiguration configuration)
        {
        ...
        {
            // Get an invoice summary via the customers 3rd party accounts provider for a date period
            List<Invoice> invoices = await _invoiceHealthService.GetInvoicesFromDate(customerId,
                DateTime.Now.AddMonths(-Convert.ToInt32(_configuration["InvoicePeriodMonths"])));
